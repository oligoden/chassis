...
func New(dbt, uri string) *Store {
	...
	_, err = db.Exec("CREATE TABLE `users` (`owner_id` int unsigned AUTO_INCREMENT,`uc` varchar(255) UNIQUE,`ts` DATETIME NULL DEFAULT CURRENT_TIMESTAMP,`username` varchar(255) NOT NULL,`pass_hash` varchar(255),`salt` varchar(255),`perms` varchar(255),`hash` varchar(255) , PRIMARY KEY (`owner_id`))")
	if err != nil {
